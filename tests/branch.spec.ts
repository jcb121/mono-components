...
describe("branch command", () => {
  ...
  it("should create a simple copy", () => {
    ...
    execSync(`npx tsx ./src/variant.tsx branch ./test-out/button ./test-out/${NAME}`);
    ...
    expect(contents).toContain(readFileSync("./test-in/button/index.tsx", "utf-8"));
  });
});
...
describe("rebase command", () => {
  ...
  it("should rebase the changes", () => {
    ...
    execSync(`npx tsx ./src/variant.tsx branch ./test-out/${BASE} ./test-out/${NAME}`);
    expect(existsSync(`./test-out/${NAME}`)).toBe(true);

    // update the base component
    const contents = readFileSync(`./test-out/${BASE}/index.tsx`, "utf-8");
    writeFileSync(`./test-out/${BASE}/index.tsx`, contents.replace("Button", "MyButton"));


    // rebase the component
    execSync(`npx tsx ./src/variant.tsx rebase ./test-out/${BASE} ./test-out/${NAME}`);
    expect(readFileSync(`./test-out/${NAME}/index.tsx`, "utf-8")).toContain("export const MyButton = () => {")
  })
...
describe("rebase command", () => {
  ...
  it("should rebase the changes", () => {
    ...
    writeFileSync(`./test-out/${NAME}/index.tsx`, newContents.replace("Button", "TheirButton"));
    ...
    expect(fileContents).toContain("<<<<<<< test-out/myTestButtonRebased2/index.tsx")
